...
export default function MensajesPsico() {
    ...
    useEffect(() => {
        async function cargar() {
            ...
            // ✅ DESPUÉS — enriquece el nombre con Firestore al cargar
            onSnapshot(qConv, async snap => {
                const convs = await Promise.all(
                    snap.docs.map(async d => {
                        const conv = { id: d.id, ...d.data() } as Conversacion;
                        // Si el nombre está vacío o es genérico, buscar el real
                        if (!conv.pacienteNombre || conv.pacienteNombre === "Paciente") {
                            try {
                                ...
                                if (pacSnap.exists()) {
                                    ...
                                    // Actualizar en Firestore para que no vuelva a pasar
                    ...
                    <>
                        ...
                        {pacsFiltrados.map(paciente => (
                            <div key={paciente.uid} onClick={() => abrirConversacion(paciente)}
                                ...
                                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                                    <Avatar nombre={paciente.nombre} fotoUrl={paciente.fotoUrl} size={40} />
                                    <div>
