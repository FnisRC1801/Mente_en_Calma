// app/(protected)/dashboard-psico/mensajes/page.tsx
"use client";
...
// 🎛️ FILTROS ACTUALIZADOS (Se quitó "Urgentes")
const FILTROS = ["Todos", "No leídos"];
...
export default function MensajesPsico() {
            ...
            snapCitas.docs.forEach(d => {
                ...
                if (!pacientesMap.has(data.pacienteId)) {
                    pacientesMap.set(data.pacienteId, {
                        uid: data.pacienteId,
                        nombre: data.pacienteNombre ?? "Paciente",
                        especialidad: data.especialidad ?? "",
                    });
                }
            });

            const pacientesConFoto = await Promise.all(
                Array.from(pacientesMap.values()).map(async (p) => {
                    try {
                        const snap = await getDoc(doc(db, "pacientes", p.uid));
                        if (snap.exists()) {
                            const data = snap.data();
                            return { ...p, nombre: data.nombre ?? p.nombre, fotoUrl: data.fotoUrl ?? undefined };
                        }
                    } catch { }
                    return p;
                })
            );
            setPacientesDisponibles(pacientesConFoto);
            ...
            onSnapshot(qConv, async snap => {
                const convs = await Promise.all(
                    snap.docs.map(async d => {
                        ...
                        if (!conv.pacienteNombre || conv.pacienteNombre === "Paciente") {
                            try {
                                const pacSnap = await getDoc(doc(db, "pacientes", conv.pacienteId));
                                if (pacSnap.exists()) {
                                    const data = pacSnap.data();
                                    conv.pacienteNombre = data.nombre ?? conv.pacienteNombre;
                                    conv.pacienteFotoUrl = data.fotoUrl ?? conv.pacienteFotoUrl;
                                    await updateDoc(doc(db, "conversaciones", conv.id), {
                                        pacienteNombre: conv.pacienteNombre,
                                        pacienteFotoUrl: conv.pacienteFotoUrl ?? null,
                                    });
                                }
                            } catch { }
                        }
    ...
    async function abrirConversacion(paciente: Paciente) {
        ...
        if (!convSnap.exists()) {
            await setDoc(convRef, {
                ...
                pacienteFotoUrl: paciente.fotoUrl ?? null,
        ...
        const conv: Conversacion = {
            ...
            pacienteId: paciente.uid,
            pacienteNombre: paciente.nombre,
            pacienteFotoUrl: paciente.fotoUrl,
            doctorEspecialidad: paciente.especialidad,
        ...
        // Sincronizamos las listas locales al momento para remover el fantasma inmediatamente
    ...
    async function handleSeleccionarConv(conv: Conversacion) {
        let nombreReal = conv.pacienteNombre ?? "Paciente";
        let fotoReal = conv.pacienteFotoUrl;

        try {
            const snap = await getDoc(doc(db, "pacientes", conv.pacienteId));
            if (snap.exists()) {
                const data = snap.data();
                nombreReal = data.nombre ?? nombreReal;
                fotoReal = data.fotoUrl ?? fotoReal;
            }
        } catch { }

        await updateDoc(doc(db, "conversaciones", conv.id), {
            pacienteNombre: nombreReal,
            pacienteFotoUrl: fotoReal ?? null,
            noLeidos_doctor: 0,
        });

        setConvActiva({ ...conv, pacienteNombre: nombreReal, pacienteFotoUrl: fotoReal });
    ...
    // Filtros de navegación actualizados (Sin Urgentes)
    const convFiltradas = conversaciones.filter(c => {
        ...
        const coincideFiltro = filtro === "Todos" ? true : c.noLeidos_doctor > 0;
    ...
    return (
        <div style={{ height: "calc(100vh - 53px)", display: "flex", fontFamily: "'Montserrat', sans-serif", background: "white" }}>
            ...
            <div style={{ width: 300, borderRight: "1px solid #e5e7eb", display: "flex", flexDirection: "column", flexShrink: 0 }}>
                ...
                <div style={{ flex: 1, overflowY: "auto" }}>
                    {convFiltradas.map(conv => {
                        const fotoReal = pacientesDisponibles.find(p => p.uid === conv.pacienteId)?.fotoUrl ?? conv.pacienteFotoUrl;
                        return (
                            <div key={conv.id} onClick={() => handleSeleccionarConv(conv)}
                                ...
                                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                                    <Avatar nombre={conv.pacienteNombre} fotoUrl={fotoReal} size={40} />
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                            <p style={{ margin: 0, fontWeight: 600, fontSize: "0.85rem", color: "#1a2e2c", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                                {conv.pacienteNombre ?? "Paciente"}
                                            </p>
                                            <span style={{ fontSize: "0.68rem", color: "#9ca3af", flexShrink: 0, marginLeft: 4 }}>
                                                {formatFechaConv(conv.ultimaFecha)}
                                            </span>
                                        </div>
                                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                            <p style={{ margin: 0, fontSize: "0.75rem", color: "#9ca3af", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                                {conv.ultimoMensaje || conv.doctorEspecialidad}
                                            </p>
                                            {conv.noLeidos_doctor > 0 && (
                                                <div style={{ width: 18, height: 18, borderRadius: "50%", background: "#4a8a85", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginLeft: 4 }}>
            ...
            {convActiva ? (
                <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                    <div style={{ padding: "12px 20px", borderBottom: "1px solid #e5e7eb", display: "flex", alignItems: "center", gap: 12, background: "white" }}>
                        <Avatar nombre={convActiva.pacienteNombre} fotoUrl={convActiva.pacienteFotoUrl} size={38} />
                        <div>
                            <p style={{ margin: 0, fontWeight: 600, fontSize: "0.92rem", color: "#1a2e2c", fontFamily: "'Poppins', sans-serif" }}>
                                {convActiva.pacienteNombre}
                            </p>
                    ...
                    <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px", display: "flex", flexDirection: "column", gap: 8, background: "#f8fafb" }}>
                        ...
                        {mensajes.map(m => {
                            ...
                            return (
                                <div key={m.id} style={{ display: "flex", justifyContent: esMio ? "flex-end" : "flex-start", alignItems: "flex-end", gap: 8 }}>
                                    {!esMio && (
                                        <Avatar nombre={convActiva.pacienteNombre} fotoUrl={convActiva.pacienteFotoUrl} size={28} />
                                    )}
                                    <div style={{
                                        ...
                                        <p style={{ margin: "4px 0 0", fontSize: "0.65rem", color: esMio ? "rgba(255,255,255,0.7)" : "#9ca3af", textAlign: "right" }}>
                                            {formatHora(m.fecha)}
                                        </p>
                                    </div>
                                </div>
